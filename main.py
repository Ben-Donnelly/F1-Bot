...
class APICalls:
	...
	def constructors_for_year(self):
		...
		if self.round:
			constructors_root = get(f"{self.base_url}{self.year}/{self.round}/constructors")
		else:
	...
	def circuts_for_year(self):
		...
		if self.round:
			circuts_root = get(f"{self.base_url}{self.year}/{self.round}/circuits")
		else:
	...
	def results(self):
		...
		results_root = get(f"{self.base_url}{self.year}/{self.round}/results")
	...
	def driver_standings_after_a_race(self):
		...
		if not self.round:
			...
			drivers_standings_root = get(f"{self.base_url}{self.year}/{self.round}/driverStandings")
	...
	def constructor_standings_after_a_race(self):
		...
		# This has date and time
			...
			constructor_standings_root = get(f"{self.base_url}{self.year}/{self.round}/constructorStandings")
	...
	def driver_standings_by_specifying_the_driver(self, driverId):
		current_drivers_standings_root = get(f"{self.base_url}drivers/{driverId}/driverStandings")
		ret_val = etree.fromstring(current_drivers_standings_root.content)
		return etree.tostring(ret_val, pretty_print=True).decode()
	...
	def main(self):
		...
		print(api_call.driver_standings_by_specifying_the_driver("bottas"))
